...
class FeatureExtractor:
    def __init__(self, input_dim, epochs=500, lr=1e-3, batch_size=512,
    ...
    def fit(self, X, y):
        ...
        for epoch in range(self.epochs):
            ...
            if self.verbose and (epoch % 10 == 0 or epoch == self.epochs - 1):
                print(f"[FeatureExtractor] Epoch {epoch + 1}/{self.epochs}, loss = {avg_loss:.6f}")
...
class CascadedModel:
    def __init__(self, imputer, non_constant_features, selector, scaler, feature_extractor, classifiers):
        ...
        self.classifiers = classifiers  # [adaboost, lgbm]
...
def main():
    ...
    lgbm.fit(X_emb_train, y_resampled)
    ...
    y_proba_lgbm = lgbm.predict_proba(X_emb_test)[:, 1]
    y_proba = (y_proba_adaboost + y_proba_lgbm) / 2
