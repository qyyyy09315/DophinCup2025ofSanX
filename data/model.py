...
FIXED_PARAMS = {
    ...
    'adasyn_n_neighbors': 5,         # 固定 ADASYN 邻居数
...
def train_and_evaluate_fixed_params(X_train_all, y_train_all, X_test, y_test):
    ...
    # --- 5. 过采样 (ADASYN) (使用固定参数) ---
    print(f"[Resampling] 应用 ADASYN 过采样 (n_neighbors={FIXED_PARAMS['adasyn_n_neighbors']})...")
    adasyn = ADASYN(random_state=SEED, n_neighbors=FIXED_PARAMS['adasyn_n_neighbors'])
    X_resampled_scaled, y_resampled = adasyn.fit_resample(X_train_brf_selected_scaled, y_train_all)
    print(f"[Resampling] ADASYN 过采样后样本数: {X_resampled_scaled.shape[0]}")
