...
# 由于sklearn没有直接的注意力机制实现，这里假设使用定制的AttentionWrapper
...
mean_precision = precision_score(y_test, y_pred)

# 计算最终得分
final_score = 100 * (0.4 * mean_recall + 0.4 * mean_auc + 0.2 * mean_precision)
...
joblib.dump(imputer, 'imputer.pkl')  # 保存数据填补器
joblib.dump(scaler, 'scaler.pkl')  # 保存特征缩放器
joblib.dump(label_encoders, 'label_encoders.pkl')  # 保存标签编码器
joblib.dump(adasyn, 'adasyn.pkl')  # 保存采样器
